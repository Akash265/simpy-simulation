...
def truck_arrival(env, resource_handler, unloading_docks, dock_list, truck_id):
    ...
    with unloading_docks.request() as dock_request:
        ...
        #env.process(handle_forklift(env, truck, available_dock, resource_handler))
        yield env.timeout(t_unload_pallat*config["forklifts_per_unload_dock"])
        if env.now > warmup_period:
            truck_unloading_times.append(truck.capacity*t_unload_pallat)
        for _ in range(config["forklifts_per_unload_dock"]-1):  # Forklifts for both unloading and storage
        ...
        if env.now > warmup_period:
            truck_unloading_times.append(truck.capacity*t_unload_pallat)
            mean_pallet_put_times.append(unloading_duration - t_unload_pallat*config["forklifts_per_unload_dock"])
